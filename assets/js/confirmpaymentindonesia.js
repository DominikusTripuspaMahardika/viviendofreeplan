...
async function showPaymentNotification(event, productPrice) {
  ...
  let serviceFee = productPrice * 0.20;
  ...
  notification.innerHTML = `
      ...
      <input id="promo-code" type="text" placeholder="Enter promo code" style="margin: 10px 0; padding: 8px; width: 80%; border: 1px solid #ccc; border-radius: 5px;" ${purchaseCount >= 5 ? 'disabled' : ''}/>
      <button id="apply-promo" style="margin-top: 10px; padding: 8px 16px; background-color: #28a745; color: white; border: none; border-radius: 8px; cursor: pointer;" ${purchaseCount >= 5 ? 'disabled' : ''}>Promo Confirmation</button>
      <div id="promo-warning" class="promo-warning" style="display: none;">
          ⚠️ Use One Promo Code ⚠️
      </div>
      <button id="confirm-payment" style="margin-top: 10px; padding: 12px 24px; background-color: #007bff; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 16px;">Purchase Confirmation</button>
  `;
