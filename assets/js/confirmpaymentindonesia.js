...
function updateCountdown() {
  const promoEndDate = new Date("March 31, 2025 23:59:59").getTime();
  const now = new Date().getTime();
  const timeLeft = promoEndDate - now;

  if (timeLeft <= 0) {
      document.getElementById("countdownEventLimit").innerHTML = "Promo telah berakhir!";
      return;
  }
  ...
  document.getElementById("countdownEventLimit").innerHTML = `${days} hari, ${hours} jam, ${minutes} menit, ${seconds} detik`;
}
